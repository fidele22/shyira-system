...
const ForwardedRequests = () => {
  ...
  const fetchForwardedRequests = async () => {
    try {
      const response = await axios.get('http://10.20.0.99:5000/api/logisticFuel');
  ...
  const handleUpdateSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://10.20.0.99:5000/api/LogisticRequest/${selectedRequest._id}`, formData);
 ...
 const handleVerifySubmit = async (e) => {
  ...
  try {
       ...
       const response = await axios.post(`http://10.20.0.99:5000/api/logisticFuel/verified/${selectedRequest._id}`);
       setSelectedRequest(response.data);
