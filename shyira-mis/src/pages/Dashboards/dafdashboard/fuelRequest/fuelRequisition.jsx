...
const FuelRequisitionForm = () => {
  ...
  useEffect(() => {
    const fetchRequisitions = async () => {
      try {
        const response = await axios.get('http://10.20.0.99:5000/api/forwardedrequests/fuel');
  ...
  const handleRequestClick = async (requestId) => {
    try {
      const response = await axios.get(`http://10.20.0.99:5000/api/forwardedrequests/fuel/${requestId}`);
  ...
  const handleSaveClick = async () => {
    try {
      const response = await axios.put(`http://10.20.0.99:5000/api/forwardedrequests/updatefuel/${selectedRequest._id}`, formData);
      setSelectedRequest(response.data);
  ...
  const handleApproveClick = async () => {
    try {
      const response = await axios.post(`http://10.20.0.99:5000/api/forwardedrequests/approvefuel/${selectedRequest._id}`, formData);
      setSelectedRequest(response.data);
