...
import '../LogisticFuelOrders/stylingfuelorders.css'
...
const ApprovedRequests = () => {
  ...
  const fetchApprovedRequests = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/LogisticRequest/rejected-item-order`);
  ...
  const handleRequestClick = async (requestId) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/LogisticRequest/rejected-item-order/${requestId}`);
    ...
    <div className={`requist ${selectedRequest ? 'dim-background' : ''}`}>
      ...
      <div className="order-navigation">
        <div className="navigation-title">
          <h2>Requisition of fuel form logistic office rejected</h2>
        </div>
        <ul>
